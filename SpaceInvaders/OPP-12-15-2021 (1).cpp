...
public:
	...
	Entity() { life = 20.0f; x = 1.0f; y = 1.0f; dx = 0.1f; dy = 0.1f; angle = 0.0f; radius = 1.0f; }
...
public:
	...
	float health;
	SpaceCraft() {
		life = 20.0f; x = 0.1f; y = 0.1f; radius = 1; angle = -90; name = "Enemy"; automatic = 1;// health = 20;
		/*dx = (float)(rand() % 8 - 4);
		if (dx == 0)*/
		dx = (rand() % 2 == 1) ? 2 : -2;
		dy = (rand() % 2 == 1) ? 2 : -2;
...
public:
	...
	void update() {
		forward0OrBackward1 = (abs(dx) == 10) ? 1 : 0;
		//dx = (forward0OrBackward1 == 0) ? -dx : dx;
		dx = 0;
		dy = 0;
...
int main() {
	...
	RenderWindow app(VideoMode(W,L), "SPACE INVADERS");
	...
	s->setAll(sEnemy, 200 * c + 20 + 100, 100 + 100, 90, 60);
	...
	while (app.isOpen()) {
		...
		for (auto e : entities)
			for (auto f : entities)
				if (e->name == "Bullet" && f->name == "Asteroid")
				...
				else if ((e->name == "Enemy" || f->name == "Enemy") && rand()%73 == 0)
					if (isCollide(e, f)) {
						Bullets* b = new Bullets();
						b->setAll(sBullet, e->x, e->y + 150, e->angle, 30);
						entities.push_back(b);
					}
					else;
